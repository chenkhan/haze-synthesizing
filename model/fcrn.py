...
if __name__ == "__main__":
    model = ResNet(layers=50, output_size=(256,256))
    model = model.cuda()
    model.eval()
    path='C:/Users/ACM/Downloads/FCRN_pytorch-master/FCRN_pytorch-master/YT_Bing_799.jpeg'
    img = Image.open(path)
    ...
    print(output.size())
    print(output[0].size())
    image = output[0].cpu().numpy()
    image=Image.fromarray(np.uint8(image[0]))
    image.show()
