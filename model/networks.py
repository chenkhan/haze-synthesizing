...
def define_Gen_d(output_size,layer=50,init_type='xavier',gpu_ids=[]):
    net = ResNet(output_size=output_size, layers=layer)
    return init_net(net, init_type, gpu_ids)
...
class Synthesizer(nn.Module):
    ...
    def forward(self,hazefree, A, beta, depth):
        t=torch.exp(-torch.abs(beta.mul(depth)))
        output=A*(torch.ones_like(t)-t)+hazefree*t
        return output
