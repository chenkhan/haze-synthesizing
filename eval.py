...
# Translated from CIEDE2000 implementation in
...
def ciede2000(lab1, lab2):
    ...
    T = 1. - 0.17 * np.cos(radians(aHP - 39)) + 0.24 * np.cos(radians(2. * aHP)) + \
        0.32 * np.cos(radians(3. * aHP + 6.)) - 0.2 * np.cos(radians(4. * aHP - 63.))
    ...
    return np.sqrt(((dLP / (sL * kL)) ** 2.) + ((dCP / (sC * kC)) ** 2.) +
                   ((dHP / (sH * kH)) ** 2.) + rT * (dCP / (sC * kC)) * (dHP / (sH * kH)))
...
def toTensor(img):
    assert isinstance(
        img, np.ndarray), 'the img type is {}, but ndarry expected'.format(
        type(img))
